...
const r = Router();
/** ---- ENV / CONFIG ---- */
const BACKEND_PUBLIC_URL = process.env.BACKEND_PUBLIC_URL || `http://localhost:${process.env.PORT || 8080}`;
...
/*  ISSUE TICKET                                                      */
/* ------------------------------------------------------------------ */
r.post("/ticket", requireUser, async (req, res) => {
    try {
        if (!SSO_PRIVATE_KEY) {
        ...
        const ticket = jwt.sign({}, SSO_PRIVATE_KEY, {
            algorithm: "RS256",
            audience: SSO_AUDIENCE,
        ...
        const redirectUrl = `${HZ_BASE}/sso/consume?ticket=${encodeURIComponent(ticket)}&ret=${encodeURIComponent(ret)}`;
...
r.post("/redeem", async (req, res) => {
    try {
        ...
        if (!ticket)
            return res.status(400).json({ ok: false, message: "Missing ticket" });
        // Verify RS256 ticket
        ...
        const appToken = jwt.sign({ sub: userId }, process.env.JWT_SECRET, { algorithm: "HS256", expiresIn: "30d" });
