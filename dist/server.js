...
const corsOpts = {
    origin: (origin, cb) => {
        if (!origin)
            return cb(null, true); // allow curl / health
        if (FRONTEND_LIST.includes(origin))
            return cb(null, true);
...
app.get("/api/v1/_probe", (_req, res) => res.json({ ok: true, where: "server.ts probe" }));
...
async function start() {
    ...
    try {
        ...
        const server = app.listen(PORT, "0.0.0.0", () => {
            ...
            console.log("[env] TBO client=%s user=%s flightBase=%s sharedBase=%s", mask(process.env.TBO_ClientId), mask(process.env.TBO_UserName), process.env.TBO_FLIGHT_BASE_URL || "(missing)", process.env.TBO_SHARED_BASE_URL || "(missing)");
        });
        // Node HTTP server timeouts
        server.setTimeout?.(SOCK_TIMEOUT_MS);
        server.requestTimeout = REQ_TIMEOUT_MS;
        server.headersTimeout = HDR_TIMEOUT_MS;
        server.keepAliveTimeout = KA_TIMEOUT_MS;
