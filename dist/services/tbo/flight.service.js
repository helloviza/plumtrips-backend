...
export async function searchFlights(input) {
    const { origin, destination, departDate, returnDate, cabinClass = 1, adults = 1, children = 0, infants = 0, nonStopOnly = false, oneStopOnly = false, preferredAirlines = [], } = input || {};
...
/* ------------------------------------------------------------------ */
export async function getFareRule(input) {
...
export async function getFareQuote(input) {
    const { traceId, resultIndex } = input || {};
    if (!traceId || resultIndex === undefined || resultIndex === null) {
        throw new Error("traceId and resultIndex are required");
    }
...
export async function bookFlight(input) {
    const { traceId, resultIndex, passengers, contact, address, gst, isLCC = false, blockedFare = false, } = input || {};
    if (!traceId || resultIndex === undefined || resultIndex === null) {
        throw new Error("traceId and resultIndex are required");
    }
    ...
    if (!fqRes)
        throw new Error("FareQuote missing Results");
    const fareByPaxType = new Map();
    ...
    const passengersWithFare = passengers.map((p) => {
        ...
        if (!bd)
            throw new Error(`FareQuote missing FareBreakdown for PaxType ${p.PaxType}`);
        const fare = {
        ...
        if (address?.AddressLine1)
            paxAddress.AddressLine1 = address.AddressLine1;
        if (address?.AddressLine2)
            paxAddress.AddressLine2 = address.AddressLine2;
        if (address?.City)
            paxAddress.City = address.City;
        if (address?.CountryCode)
            paxAddress.CountryCode = address.CountryCode;
