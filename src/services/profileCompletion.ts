// apps/backend/src/services/profileCompletion.ts
...
function isSnoozed(key: StepKey, user: IUser): boolean {
  const list = user.profileCompletion?.snoozed || [];
  const now = Date.now();
  return list.some((s: { key: string; expiresAt: Date }) => 
    s.key === key && new Date(s.expiresAt).getTime() > now
  );
}

export function computeProfileCompletion(user: IUser): CompletionResponse {
  ...
  items.push(
    ...
    { key: "verify_mobile", label: "Mobile verified", weight: weights.verify_mobile, completed: !!user.mobileVerified },
    { key: "verify_email", label: "Email verified", weight: weights.verify_email, completed: !!user.emailVerified },
  ...
  const snoozed = (user.profileCompletion?.snoozed || []).map(
    (s: { key: string }) => s.key
  );
