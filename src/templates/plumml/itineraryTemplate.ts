      ...
      ${day.activities
        ...
        <div class="activity">
          <div class="activity-text">
            ...
            <div class="activity-place">${a.place || ""}</div>
...
<body>
  ...
  <div class="section">
    ...
    <div class="card">
      <div>
        <div class="card-name">Outbound: ${outboundFlight.airline} ${outboundFlight.flightNumber}</div>
        <div class="card-meta">${slots.originAirportCode || "Origin"} → ${slots.destinationAirportCode || "Destination"} &nbsp;|&nbsp; ${slots.departDate}</div>
        <div class="card-meta">${outboundFlight.stops === 0 ? "Non-stop" : `${outboundFlight.stops} stop(s)`} | Departs ${outboundFlight.departureTime || "N/A"}, Arrives ${outboundFlight.arrivalTime || "N/A"}</div>
      </div>
    </div>
    <div class="card">
      <div>
        <div class="card-name">Return: ${returnFlight.airline} ${returnFlight.flightNumber}</div>
        <div class="card-meta">${slots.destinationAirportCode || "Destination"} → ${slots.originAirportCode || "Origin"} &nbsp;|&nbsp; ${slots.returnDate}</div>
        <div class="card-meta">${returnFlight.stops === 0 ? "Non-stop" : `${returnFlight.stops} stop(s)`} | Departs ${returnFlight.departureTime || "N/A"}, Arrives ${returnFlight.arrivalTime || "N/A"}</div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <table class="price-table">
      <tr><td>Outbound flight (${slots.adults + slots.children} pax)</td><td style="text-align:right">${formatINR(combo.outboundFlight.price * (slots.adults + slots.children))}</td></tr>
      <tr><td>Return flight (${slots.adults + slots.children} pax)</td><td style="text-align:right">${formatINR(combo.returnFlight.price * (slots.adults + slots.children))}</td></tr>
