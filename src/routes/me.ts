...
r.get("/whoami", (req: AuthedRequest, res) => {
  res.json({ ok: true, userId: req.userId, sessionId: req.sessionId });
});

/* =========================================================================
   Helpers: Profile Completion computation (no external deps)
   ========================================================================= */
...
interface CompletionResponse {
  ...
  nextStep?: {
    ...
    reason: string;
    ctaText: string;
    target: string; // route or action token e.g., "/account/profile#passport" or "action:openOtp"
  };
...
const reasons: Record<StepKey, string> = {
...
function isSnoozed(key: StepKey, user: any): boolean {
  const list: Array<{ key: string; expiresAt: Date | string }> =
    user?.profileCompletion?.snoozed || [];
  const now = Date.now();
  return list.some(
    (s) => s.key === key && new Date(s.expiresAt as any).getTime() > now
  );
}

function computeProfileCompletion(user: any): CompletionResponse {
  ...
  const paymentMethods =
    user.paymentMethods ?? p.paymentMethods ?? [];

  const twoFactorEnabled =
    user?.twoFactor?.enabled ?? p?.twoFactor?.enabled ?? false;

  const backupEmail = user.backupEmail ?? p.backupEmail;

  const coTravellers =
    user.coTravellers ?? p.coTravellers ?? [];
  ...
  items.push(
    { key: "name", label: "Name", weight: weights.name, completed: hasName },
    {
      key: "verify_mobile",
    ...
    { key: "dob", label: "Date of birth", weight: weights.dob, completed: hasDob },
    { key: "gender", label: "Gender", weight: weights.gender, completed: hasGender },
    { key: "avatar", label: "Profile photo", weight: weights.avatar, completed: hasAvatar }
  );
  ...
  items.push(
    { key: "address", label: "Primary address", weight: weights.address, completed: hasAddress },
    { key: "passport", label: "Passport", weight: weights.passport, completed: hasPassport },
    { key: "emergency_contact", label: "Emergency contact", weight: weights.emergency_contact, completed: hasEmergency },
    { key: "prefs", label: "Travel preferences", weight: weights.prefs, completed: hasPrefs },
    { key: "gst", label: "GST / Business details", weight: weights.gst, completed: hasGst }
  );
  ...
  items.push(
    { key: "payment", label: "Saved payment method", weight: weights.payment, completed: hasPayment },
    { key: "2fa", label: "Two-factor authentication", weight: weights["2fa"], completed: has2fa },
    { key: "backup_email", label: "Backup email", weight: weights.backup_email, completed: hasBackup }
  );

  // Experience
  const hasCoTrav = Array.isArray(coTravellers) && coTravellers.length > 0;
  items.push(
    { key: "co_traveller", label: "Co-traveller", weight: weights.co_traveller, completed: hasCoTrav }
  );
  ...
  const incomplete: Record<string, CompletionItem> = {};
  for (const it of items) {
    if ((priority as string[]).includes(it.key as string) && !it.completed) {
      incomplete[it.key] = it;
    }
  }

  let nextStep: CompletionResponse["nextStep"] | undefined;
  for (const k of priority) {
    if (!incomplete[k]) continue;
    if (isSnoozed(k, user)) continue;
    nextStep = {
      key: k,
      label: labels[k],
      reason: reasons[k],
  ...
  const snoozed = (user?.profileCompletion?.snoozed || []).map((s: any) => s.key);
...
r.put("/profile", async (req: AuthedRequest, res) => {
...
// CO-TRAVELLERS
r.get("/co-travellers", async (req: AuthedRequest, res) => {
  const user = await User.findById(req.userId, { coTravellers: 1 }).lean();
  res.json({ ok: true, items: user?.coTravellers || [] });
});

r.post("/co-travellers", async (req: AuthedRequest, res) => {
  const doc = req.body || {};
  const user = await User.findByIdAndUpdate(
    req.userId,
    { $push: { coTravellers: doc } },
    { new: true, fields: { coTravellers: { $slice: -1 } } }
  ).lean();
  const created = user?.coTravellers?.[user.coTravellers.length - 1];
  res.json({ ok: true, item: created });
});

r.put("/co-travellers/:id", async (req: AuthedRequest, res) => {
  const { id } = req.params;
  const update = req.body || {};
  await User.updateOne(
    { _id: req.userId, "coTravellers._id": id },
    { $set: Object.fromEntries(Object.entries(update).map(([k, v]) => [`coTravellers.$.${k}`, v])) }
  );
  res.json({ ok: true });
});

r.delete("/co-travellers/:id", async (req: AuthedRequest, res) => {
  const { id } = req.params;
...
// PASSWORD
r.put("/password", async (req: AuthedRequest, res) => {
  const { oldPassword, newPassword } = req.body || {};
  if (!newPassword || newPassword.length < 8) return res.status(400).json({ ok: false, message: "Invalid new password" });
...
// SESSIONS
r.get("/sessions", async (req: AuthedRequest, res) => {
  const sessions = await Session.find({ userId: req.userId, revokedAt: { $exists: false } })
    .sort({ createdAt: -1 })
    .lean();
  const currentSid = req.sessionId;
  const items = sessions.map((s: any) => ({ ...s, current: String(s._id) === currentSid }));
  res.json({ ok: true, items });
});

r.post("/sessions/:id/revoke", async (req: AuthedRequest, res) => {
  const { id } = req.params;
...
r.get("/profile-completion", async (req: AuthedRequest, res) => {
  try {
...
r.post("/profile-snooze", async (req: AuthedRequest, res) => {
  try {
    const { key } = req.body || {};
    ...
    await User.updateOne(
      { _id: req.userId },
      { $pull: { "profileCompletion.snoozed": { key } } },
      { strict: false }
    );

    await User.updateOne(
      { _id: req.userId },
      { $push: { "profileCompletion.snoozed": { key, expiresAt } } },
      { strict: false }
    );
